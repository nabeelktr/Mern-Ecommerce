...
const EditAddress = ({}) => {
  ...
  const handleSubmit = async(values, action) => {
    await Axios.post('/editAddress',values);
    toast.success('Address updated successful');
    navigate('/profile/viewAddress')
  }
        ...
        >
          {({ isSubmitting }) => (
            <Form>
              ...
              >
                <Typography className="mr-2 uppercase p-1 font-bold text-gray-100 text-sm">Save Changes</Typography>
              </button>
            </Form>
          )}
        </Formik>
