      ...
      >
        ...
        <Form className='flex w-1/2'>
          ...
          {isSubmitting ? <div className="flex justify-center h-6 p-2  absolute bottom-4 right-4">
                ...
                </div> :<button type="submit" className="p-2  absolute bottom-4 right-4 bg-blue-500 hover:bg-gray-400 text-white font-bold py-2 px-4 rounded inline-flex items-center" >Add Product</button>
          }
        </Form>
        )}
        </Formik>
          ...
          <input
            ...
            <div className='flex flex-row p-4 overflow-auto mb-6'>
                ...
                {images[0] && images.map((img,i) => (
                  <img src={URL.createObjectURL(img)} key={i} alt="Uploaded Image" height="400px" width="300px" className='m-2' onClick={uploadFiles}/>
                  ))}
