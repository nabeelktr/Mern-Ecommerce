...
const ProductTable = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (info) => (
        <>
          {info.getValue() &&
            info.getValue().map((variant, q) => <p key={q}>{variant.qty}</p>)}
        </>
      ),
    },
    {
    ...
    },
    {
      ...
      cell: (info) => (
        ...
        onClick={() =>
          ...
          className="flex w-full items-center py-2 px-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white text-gray-700 dark:text-gray-200"
        >
          <svg
  ...
  return <BasicTable datas={products} columns={columns} type={"products"} />;
};
